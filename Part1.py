...
def __dyna_q(q_value, model, maze, dyna_params):
    ...
    for run in tqdm(range(runs)):
        for i, planning_step in enumerate(planning_steps):
            ...
            for ep in range(episodes):
                steps[i, ep] += __dyna_q(q_value, model, dyna_maze, dyna_params)
    steps /= runs
    for i in range(len(planning_steps)):
        plt.plot(steps[i, :], label='%d planning steps for Dyna-Q' % (planning_steps[i]))
    plt.xlabel('episodes')
    plt.ylabel('steps per episode')
    plt.legend()
    plt.savefig('DynaQ_Plot.png')
    plt.close()
